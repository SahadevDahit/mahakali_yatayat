...
async function getData() {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_URL}/company_in_ex_form/get`,
    {
      next: {
        revalidate: 10,
      },
...
const Company_in_ex = ({ id }) => {
  ...
  return (
    <div className="w-100 h-100 container">
      <h1 className="text-center">Company In-Ex Records</h1>
      ...
      <div className="col-3">
        <Button>
          <Link
            variant="dark"
            className="float-end "
            href={{ pathname: "/app/company_in_ex", query: { id: "000" } }}
          >
            Add
          </Link>
        </Button>
